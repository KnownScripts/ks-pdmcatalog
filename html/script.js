...
var ShowroomFunctions = {
    ...
    GetCars: function () {
        $.post('https://ks-pdmcatalog/qb-showroom:showroomChangeCar', JSON.stringify({ model: cars[selectedId].model }), function (cb) {
...
$(document).ready(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.meta.message = "done") {
            ...
            $.post('https://ks-pdmcatalog/qb-showroom:application-closed', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
$(document).on("click", "#purchase", function () {
    canPress = false
    $.post('https://ks-pdmcatalog/showroomPurchaseCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
